...
export class MmorpgComponent {
  ...
  async cargarPosts(){
    this.loading = true;
    let comunityPost = await this.restSvc.getComunityPost("mmorpg",this.page, this.limit);
  ...
  async nuevoMensaje(){
    ...
    if(this.contenido !== ""){
      let mensajeRespuesta = await this.restSvc.newPost(this.id,this.contenido,"mmorpg")
